    ...
    buttonCreate.addEventListener("click", function () {
        ...
        dlg.new(e => {
            if (e === "add") {
                ...
                const [block, name, editBtn, graph] = create();
                block.dataset.cost = res.cost;
                block.dataset.term = res.term;
                block.dataset.rate = res.rate;
                block.dataset.first = res.first;
                block.dataset.name = res.name;
                editBtn.addEventListener('click', type => {
                    dlg.change(block.dataset.cost, block.dataset.rate, block.dataset.term, block.dataset.first, block.dataset.name, type => {
                        if (type === "change") {
                            name.innerText = dlg.curem.name
                            const [prib, payUp, summ, month, rate] = [...block.querySelectorAll('.info__div-add-data')].map(e => e.children.item(1))

                            block.dataset.cost = dlg.curem.cost;
                            block.dataset.term = dlg.curem.term;
                            block.dataset.rate = dlg.curem.rate;
                            block.dataset.first = dlg.curem.first;
                            block.dataset.name = dlg.curem.name;

                            prib.innerText = `${dlg.curem.percentSumm.toFixed(2).replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 ')} ₽`;
                            payUp.innerText = ` ${dlg.curem.payUp.toFixed(2).replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 ')} ₽`;
                            summ.innerText = `${dlg.curem.cost.toFixed(2).replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 ')} ₽`;
                            month.innerText = `${dlg.curem.term} мес.`;
                            rate.innerText = `${dlg.curem.rate}% в мес.`;

                            graph.drawGraph(dlg.curem)
                        }
                    })
                });
            }
        ...
        function create() {
            ...
            */
            ...
            // editButoon.addEventListener("click", function (event) {
            ...
            // editableElm.contentEditable = true;
            // });
            return [newCreatedBlock, newCreatedHeaderName, editButoon, graph];
            //form.reset();
        }
    });
});